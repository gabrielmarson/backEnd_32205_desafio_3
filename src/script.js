...
class ProductMananger {
  ...
  getProduct = async () => {
    if (fs.existsSync(this.path)) {
      ...
      console.log(`No hay ningún producto disponible`);
    }
  };
  ...
  uptadeProduct = async (id, propiedadActualizadas) => {
    if ((id, propiedadActualizadas)) {
      ...
      console.log(`Completa todas las propiedades para actualizar el producto`);
    }
  };
...
const productosDisponibles = new ProductMananger("listaDeProductos.json");

module.exports.products = productosDisponibles;